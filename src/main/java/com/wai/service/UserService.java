...
public class UserService {
    ...
    public List<EnneagramTestDto> getUserEnneagramTests(UserRequestDto userRequestDto) {
       List<EnneagramTest> enneagramTests = userRepository.getUserEnneagramTests(userRequestDto.getUserKey()).orElse(new ArrayList<>());
       return enneagramTests.stream().map(EnneagramTest::toDto).collect(Collectors.toList());
    }
}
